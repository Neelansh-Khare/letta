...
def run_longmemeval(args):
    ...
    for item_idx, item in enumerate(tqdm(data, desc="Processing LongMemEval items")):
        print(f"[LongMemEval] Item {item_idx + 1}/{total_items}: creating agent")
        ...
        for session_idx, session in enumerate(sessions, start=1):
            ...
            print(
                f"[LongMemEval] Item {item_idx + 1}/{total_items}: "
                f"session {session_idx}/{len(sessions)} loaded ({len(session)} messages)"
            )

        question = item["question"]
        ground_truth = item["answer"]
        print(f"[LongMemEval] Item {item_idx + 1}/{total_items}: answering final question")
